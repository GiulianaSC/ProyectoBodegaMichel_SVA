...
namespace api_bodega.Controllers
{
    ...
    public class ClienteController : ControllerBase
    {
        ...
        {
            return await context.Cliente.Include(x => x.distrito).ToListAsync();
        }
        ...
        public async Task<ActionResult> add(Cliente c)
        {
            ...
            var distritoexiste = await context.Distrito.AnyAsync(x => x.idDistrito == c.idDistrito);

            if (!distritoexiste)
            {
                return BadRequest($"No existe el distrito con codigo: {c.idDistrito}");
            }
            context.Add(c);
